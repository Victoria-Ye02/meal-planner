...
export function RecipeCard({
  ...
  const visual = pickRecipeVisual(recipe.title);
  const RecipeIcon = visual.icon;

  return (
    <div className="flex flex-col overflow-hidden rounded-card border border-border bg-surface shadow-sm transition-shadow duration-200 ease-out-quart hover:shadow-md">
      <div
        className={`flex h-20 items-center justify-center ${visual.gradientClassName}`}
      >
        <RecipeIcon
          size={36}
          stroke={1.75}
